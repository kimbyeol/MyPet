...
$(document).ready(function () {
	...
	if(${!empty login_info }){
		<c:forEach var="dto" items="${vo}">
			...
			titletag += 'onclick="return show_hide_box(this, 460, 250, \'${dto.s_title}\', \'${dto.s_content}\', \'${dto.s_datestring}\', ${dto.s_id})">${dto.s_title}';
			titletag += '</div>';
	...
	/* location.href="main.cal?s_name=${login_info.nickname}";  */
});
...
function show_hide_box(atag, width, height, s_title, s_content, s_date, s_id) {
	...
	var contentTag = '<a class="ic_cancel">닫기</a>';
		...
		contentTag += '<a style="margin-right: 10px;" class="btn-empty" id="cancel" onclick="sc_delete('+ s_id +')">삭제</a>'
	...
	$('.up_content').val(s_content.replace(/<br>/g, '\n'));
...
function sc_insert() {
	if(${!empty login_info }){			
		if($.trim($('.in_title').val()) == ''){
			alert('일정명을 입력해주세요!');
			$('.in_title').focus();
			return false;	
		}else if($.trim($('.in_content').val()) == ''){
			alert('일정메모를 입력해주세요!');
			$('.in_content').focus();
			return false;
		}else{
			var nic="${login_info.nickname}";		
			$.ajax({
				type: 'post',
...
function sc_update(s_id) {
	if($.trim($('.up_title').val()) == ''){
		alert('일정명을 입력해주세요!');
		$('.up_title').focus();
		return false;	
	}else if($.trim($('.up_title').val()) == ''){
		alert('일정메모를 입력해주세요!');
		$('.up_content').focus();
		return false;
	}else{
		...
		$.ajax({
			...
			data: { sc_title: $('.up_title').val(),
					sc_content: $('.up_content').val(),
					sc_id: s_id	
			},
			success: function (data) {
				if(data == 'succ'){
...
//일정삭제
function sc_delete(s_id) {
	
	if(confirm('삭제하시겠습니까?')){
		$.ajax({
			type: 'post',
			url: 'calendar/main/sc_delete',
			data: { sc_id: s_id },
			success: function (data) {
				if(data == 'succ'){
					alert('일정이 삭제되었습니다');
